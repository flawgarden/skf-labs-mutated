...
#Mutation info: Insert template from templates-db/languages/python/sensitivity/record/record.tmt with name nested_record_positive
#Used extensions:
#Program:
from .Record import *
...
def home():
    ...
    first = SimpleRecord(ip_address)
    second = SimpleRecord("")
    nested = NestedRecord(first, second)
    ip_address = nested.a.t
    ip_address = ip_address.replace(";","")
    ip_address = ip_address.replace("&","")
