...
#Used extensions: EXPR_bool -> ~[EXPR_str@1]~ is not None and ~[EXPR_str@1]~[~[CONST_int]~] == ~[CONST_str]~ | EXPR_str -> ~[EXPR_str]~.strip() | EXPR_str -> ~[EXPR_str]~.upper() | EXPR_str -> ~[EXPR_str]~.replace(~[EXPR_str]~, ~[EXPR_str]~) | EXPR_str -> ~[EXPR_str]~.replace(~[EXPR_str]~, ~[EXPR_str]~)
#Program:
from .Record import *
...
def home():
    ...
    try:
        if "wkktg" is not None and "wkktg"[-2741] == "urnru":
            raise Exception1("mzmfa".replace("wygce", "rzjtv").strip())
        else:
            raise Exception2("jvrgq".replace(ip_address, "rzvqn").upper())
