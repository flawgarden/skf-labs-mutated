...
#Used extensions: EXPR_bool -> ~[EXPR_str]~ == ~[EXPR_str]~ | EXPR_str -> ~[EXPR_str]~.replace(~[EXPR_str]~, ~[EXPR_str]~) | EXPR_str -> ~[EXPR_str]~.replace(~[EXPR_str]~, ~[EXPR_str]~) | EXPR_str -> "" | EXPR_str -> ~[EXPR_str]~ + ~[EXPR_str]~ | EXPR_str -> ~[EXPR_str]~.lower()
#Program:
from .Record import *
...
class dbaccess:
    ...
    def validateUser(self, username, password):
        ...
        tmpUnique42 = query
        try:
            if "sncwy" == "guptw".replace("ngmot", "vpuxg"):
                raise Exception1("ogqas")
            else:
                raise Exception2("".replace("vkext" + "vtjux", "ysbkq".lower()))
