...
#Mutation info: Insert template from templates-db/languages/python/sensitivity/concurrency/concurrency.tmt with name two_switching_tasks_concurrent_positive
#Used extensions:
#Program:
...
def home():
    ...
    task1 = SwitchingTask(w, text_input)
    task2 = SwitchingTask(w, text_input)
    task1.start()
    task2.start()
    try:
        task1.join()
