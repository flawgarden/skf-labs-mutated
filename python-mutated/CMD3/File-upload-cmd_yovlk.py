...
#Mutated file region: 74, null, null, null
...
#Used extensions: EXPR_bool -> ~[EXPR_str]~.endswith(~[EXPR_str]~) | EXPR_str -> ~[EXPR_str]~.strip() | EXPR_str -> ~[EXPR_str]~.upper() | EXPR_str -> ~[EXPR_str]~.strip()
#Program:
from .Record import *
...
def index():
    if request.method == 'POST':
        ...
        tmpUnique42 = file
        try:
            if "imxqw".endswith("pztmh"):
                raise Exception1("fofdk".strip().strip())
            else:
                raise Exception2("ctgiq".upper())
