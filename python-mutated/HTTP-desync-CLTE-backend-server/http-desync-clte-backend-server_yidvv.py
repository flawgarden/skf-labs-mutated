...
#CodeQL analysis results: [99, 36, 23, 22, 73]
...
#Used extensions: EXPR_bool -> ~[EXPR_str]~.endswith(~[EXPR_str]~) | EXPR_str -> ~[EXPR_str]~.replace(~[EXPR_str]~, ~[EXPR_str]~) | EXPR_str -> ~[EXPR_str]~[~[EXPR_int]~:~[EXPR_int]~]
#Program:
from .Record import *
...
class ContentLengthFrontEndServer(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if "nwcdc".replace("mxnxp", "asfpo").endswith(CRLF[27500:-15833]):
            file = ""
